...
const data: Record<string, { title: string; desc: string; ops: { op: string; time: string; desc: string; color: string }[] }> = {
  ...
  "big-o": {
    ...
    ops: [
      { op: "O(1) — Constant", time: "O(1)", desc: "Same time regardless of input size. Array index, hash lookup", color: "var(--el-success)" },
      { op: "O(log n) — Logarithmic", time: "O(log n)", desc: "Halves the problem each step. Binary search on 1B items = 30 steps", color: "var(--el-muted)" },
      { op: "O(n) — Linear", time: "O(n)", desc: "Proportional to input size. Scan 1M items = 1M steps", color: "var(--el-muted)" },
      { op: "O(n log n)", time: "O(n log n)", desc: "Optimal comparison sort. Python's Timsort — 1M items ≈ 20M comparisons", color: "var(--el-muted)" },
      { op: "O(n²) — Quadratic", time: "O(n²)", desc: "Nested iterations. Bubble sort on 1M items = 5×10¹¹ operations", color: "var(--el-error)" },
      { op: "O(2ⁿ) — Exponential", time: "O(2ⁿ)", desc: "Doubles with each addition. Fib(100) naive = more atoms than universe", color: "var(--el-error)" },
    ],
  },
  "binary-search": {
  ...
  "dynamic-programming": {
    title: "DP Costs",
    desc: "DP trades memory for speed. Exponential → polynomial is the typical gain. The key is choosing the right state definition.",
    ops: [
      { op: "Memoization (top-down)", time: "O(states)", desc: "Each state computed once. Recursion + cache dict.", color: "var(--el-success)" },
      { op: "Tabulation (bottom-up)", time: "O(states)", desc: "Iterative table fill. Often faster — no recursion overhead.", color: "var(--el-success)" },
      { op: "Space (memo/tab)", time: "O(states)", desc: "Can often be optimized: Fibonacci O(n)→O(1), knapSack 2D→1D", color: "var(--el-muted)" },
      { op: "Naive recursive fib", time: "O(2ⁿ)", desc: "fib(50) = 2⁵⁰ ≈ 10¹⁵ calls. DP: 50 calls. 20 trillion × faster", color: "var(--el-error)" },
      { op: "LCS / Edit Distance", time: "O(m·n)", desc: "Two-string DP. 1000 chars = 1M cells — practical for real-world sequences", color: "var(--el-muted)" },
    ],
  },
  "fenwick-tree": {
  ...
  lists: {
    ...
    ops: [
      { op: "Index [i]", time: "O(1)", desc: "Direct element access — base + offset", color: "var(--el-success)" },
      { op: "Append", time: "O(1)*", desc: "Amortized — ~6% overallocation in CPython", color: "var(--el-success)" },
      { op: "Extend / += ", time: "O(k)", desc: "k = length of added iterable", color: "var(--el-muted)" },
      { op: "Pop from end", time: "O(1)", desc: "Remove last element", color: "var(--el-success)" },
      { op: "Insert at i", time: "O(n)", desc: "Shifts n-i elements right", color: "var(--el-error)" },
      { op: "Delete lst[i]", time: "O(n)", desc: "Shifts n-i elements left", color: "var(--el-error)" },
      { op: "Search x in lst", time: "O(n)", desc: "Linear scan — compares each element", color: "var(--el-error)" },
      { op: "Sort (.sort)", time: "O(n log n)", desc: "Timsort — hybrid merge+insertion sort", color: "var(--el-muted)" },
      { op: "Copy [:]", time: "O(n)", desc: "Shallow copy — copies n references", color: "var(--el-muted)" },
    ],
  },
  mathematical: {
  ...
  sorting: {
    title: "Sorting Costs",
    desc: "O(n log n) is the theoretical best for comparison-based sorting. Python's built-in sort uses Timsort — O(n) on partially-sorted data.",
    ops: [
      { op: "Bubble / Selection", time: "O(n²)", desc: "Simple but slow. 10K items = 50M swaps", color: "var(--el-error)" },
      { op: "Timsort (Python)", time: "O(n log n)", desc: "Hybrid merge+insertion. Adaptive — O(n) for nearly-sorted data", color: "var(--el-success)" },
      { op: "Merge / Quick / Heap", time: "O(n log n)", desc: "Optimal for general comparison sorting", color: "var(--el-success)" },
      { op: "Counting / Radix", time: "O(n+k)", desc: "Non-comparison — exploits integer keys for linear time", color: "var(--el-muted)" },
      { op: "Space (Merge sort)", time: "O(n)", desc: "Auxiliary array needed. Quick sort is O(log n) in-place", color: "var(--el-muted)" },
      { op: "Stability", time: "varies", desc: "Stable sorts preserve equal-element order. Timsort, merge sort are stable. Quick sort is not.", color: "var(--el-muted)" },
    ],
  },
  "space-complexity": {
